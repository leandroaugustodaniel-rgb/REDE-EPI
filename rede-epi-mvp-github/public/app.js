...
const brandLogo = (variant = "") => `
  <img class="brand-logo ${variant}" src="/assets/logo-rede-epi-transparent.png" alt="REDE EPI" />
`;
    ...
    <section class="login-shell">
      <div class="login-visual">
        <div class="side-head">
          ...
          <div><strong>REDE EPI</strong><small>Gestao de pessoas</small></div>
        </div>
        <div>
          <h1>Gestao padronizada, equipe mais segura.</h1>
          <p>Um MVP para acompanhar colaboradores, feedbacks, DISC, desenvolvimento e estrutura organizacional com a identidade operacional da REDE EPI.</p>
        </div>
...
async function renderDashboard() {
  ...
  workspace().innerHTML = `
    ...
    <section class="grid stats">
      ...
      ${stat("Ativos", dashboard.totals.active || 0)}
...
function renderEmployees() {
  workspace().innerHTML = `
    ...
    <div class="table-wrap">
      <table>
        <thead><tr><th>Nome</th><th>Departamento</th><th>Cargo</th><th>Unidade</th><th>Lider</th><th>Status</th><th></th></tr></thead>
        <tbody>
  ...
  workspace().innerHTML = `
    ...
    <section class="panel">
      <div class="profile-head">
        ...
        <span class="badge">${escapeHtml(employee.status)}</span>
      </div>
  ...
  modal(`
    <form id="employeeForm" class="modal-card">
      ...
      <div class="form-grid">
        ...
        <label>Status<select name="status"><option ${item.status === "Ativo" ? "selected" : ""}>Ativo</option><option ${item.status === "Inativo" ? "selected" : ""}>Inativo</option></select></label>
        <label>Departamento<select name="department_id">${optionList(state.options.departments, item.department_id)}</select></label>
        <label>Cargo<select name="role_id">${optionList(state.options.roles, item.role_id)}</select></label>
        <label>Unidade<select name="unit_id">${optionList(state.options.units, item.unit_id)}</select></label>
        <label>Lider<select name="leader_id">${optionList(state.options.leaders.filter((leader) => leader.id !== id), item.leader_id)}</select></label>
